...
import { collectConfigurableFields } from '../../lib/stepData';
...
export const StepDataEditor = () => {
  ...
  useEffect(() => {
    fetch('/api/admin/step-data', { headers: authHeaders() })
      ...
      .then((data) => setOverrides(data.stepData || {}))
  ...
  const save = async () => {
    ...
    try {
      const res = await fetch('/api/admin/step-data', {
        ...
        body: JSON.stringify({ stepData: overrides }),
      });
      ...
      setMessage('Datos guardados. Los colaboradores los verán al recargar su enlace.');
    } catch {
    ...
    <Card>
      ...
      <div className="space-y-3 px-5 py-5">
        ...
        {byModule.map(([moduleId, group]) => {
          ...
          const scenario = resolveScenario(moduleId, previewModules);
          return (
                ...
                <div className="space-y-4 px-4 py-4">
                  <div className="grid gap-4 sm:grid-cols-2">
                    {group.fields.map((field) => (
                      <label key={`${field.stepId}.${field.path}`} className="block">
                        <span className="mb-1 block text-xs font-semibold uppercase tracking-wide text-ink-muted">
                          {field.label}
                        </span>
                        <Input value={field.value} onChange={(e) => updateField(field, e.target.value)} />
                      </label>
                    ))}
                  </div>
