...
export const modulesData: ModuleData[] = [
  ...
  },
  {
    ...
    steps: [
      ...
      { id: 'm3-s7', allowedTargets: ['pay-btn-apply', 'pay-btn-vuelto'], instruction: 'Haz clic en el botón "Vuelto" para confirmar la entrega del vuelto.', targetId: 'pay-btn-vuelto', screenId: 'payment' },
      { id: 'm3-s8', allowedTargets: ['pay-btn-apply', 'pay-btn-vuelto'], instruction: 'Imprime y actualiza el documento para finalizar.', targetId: 'pay-btn-print-update', screenId: 'payment', action: (s) => { s.cart = []; s.currentCustomer = null; s.payments = []; s.takeAmount = ''; s.vueltoGiven = false; } },
  ...
  },
  {
    ...
    steps: [
      ...
      { id: 'm5-s7', allowedTargets: ['pay-btn-apply', 'pay-btn-vuelto'], instruction: 'Imprime y actualiza el documento para finalizar.', targetId: 'pay-btn-print-update', screenId: 'payment', // El aviso tiene que decir CÓMO se arregla, no solo qué falta. Si el
      // documento ya quedó cubierto por un solo pago, aplicar el que falta no
      // hace nada —no queda saldo pendiente—: hay que anular y volver a cobrar.
      validator: (s) => !cobradoCon(s, 'Efectivo') ? 'Falta la parte que la clienta paga en efectivo. Si el documento ya quedó cubierto con un solo pago, anúlalo y cobra otra vez: primero el efectivo, después la tarjeta.' : !cobradoCon(s, 'Tarjeta de Crédito') ? 'El saldo va con la tarjeta de crédito. Si el efectivo cubrió todo el documento, anula ese pago y cobra solo la parte que dice el caso.' : true, action: (s) => { s.cart = []; s.currentCustomer = null; s.payments = []; s.takeAmount = ''; s.cardType = ''; s.tipoProcesamiento = 'Manual'; s.e115 = ''; s.noAutorizacion = ''; s.selectedPaymentMethod = 'Efectivo'; } },
  ...
  },
  {
    ...
    steps: [
      ...
      { 
        ...
        data: { efectivoEntregado: '200.00' },
        dataLabels: { efectivoEntregado: 'Efectivo con el que paga la empresa' },
  ...
  },
  {
    ...
    steps: [
      ...
      { id: 'm14-s4', instruction: 'Ingresa el monto del fondo de caja de apertura (150.00).', targetId: 'input-fondo-caja', targetValue: '150.00', screenId: 'registro' },
